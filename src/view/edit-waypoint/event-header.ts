...
function TEMPLATE() {
  const { dateFrom, dateTo, destination, type } = DEFAULT_DESTINATION;

  const timeStart = dateFrom.format('DD/MM/YY HH:mm');
  const timeEnd = dateTo.format('DD/MM/YY HH:mm');
...
export default class EventHeader extends View<HTMLDivElement> {
  get template() {
    return TEMPLATE();
  }
}
